...
output_folder = "Bilder/StableDiffusionDistilled/"
#####################################################################################################
...
pipeline = AutoPipelineForImage2Image.from_pretrained("nota-ai/bk-sdm-small", use_safetensors=True)
...
for input_path in glob.glob(os.path.join(input_folder, '**', '*.jpg'), recursive=True):
    ...
    base_name = os.path.splitext(os.path.basename(input_path))[0]  # Extract the base name without extension
