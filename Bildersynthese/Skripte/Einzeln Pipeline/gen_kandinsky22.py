...
output_folder = "Bilder/Kandinsky2.2/"
...
#makedirs if output doesnt exist (yet)
os.makedirs(output_folder, exist_ok=True)
input_folder=os.path.abspath(input_folder)

# Initialize the pipeline without specifying torch_dtype to float16, and without xFormers and CPU offload optimizations
pipeline = AutoPipelineForImage2Image.from_pretrained(
    "kandinsky-community/kandinsky-2-2-decoder", use_safetensors=True
)
...
for input_path in glob.glob(os.path.join(input_folder, '**', '*.jpg'), recursive=True):
    ...
    for strength in strength_values:
        try:
            # Generate the image with the current strength value and prompt
