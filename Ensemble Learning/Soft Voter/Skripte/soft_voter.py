...
plt.tight_layout()
plt.savefig("output/soft_voter_base_confusion_matrix.png", dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig("output/soft_voter_base_confusion_matrix_normalised.png", dpi=300)
plt.close()
print(f"{datetime.now()}: Confusion matrix for Hard Voter saved.")
...
with open('output/soft_voter_base_logged_data.json', 'w') as f:
...
excel_path='output/soft_voter_base_predictions_visualization.xlsx'
...
wb = load_workbook("output/soft_voter_base_predictions_visualization.xlsx")
ws = wb.active
...
prediction_cols = ['C', 'D', 'E', 'F', 'G', 'H', 'I']  # Add more columns as needed based on your file
...
wb.save("output/soft_voter_base_predictions_visualization_formatted.xlsx")
from openpyxl import load_workbook
from openpyxl.styles import PatternFill
from openpyxl.utils import get_column_letter

# Load your workbook and select the first sheet
wb = load_workbook("output/soft_voter_base_predictions_visualization_formatted.xlsx")
ws = wb.active

# Assuming column B contains the true labels
true_label_col = 'B'
# Assuming column C contains the hard voter predictions, and columns D onwards are individual model predictions
prediction_cols = ['C', 'D', 'E', 'F', 'G', 'H', 'I']  # Adjust based on your file
...
# Save the workbook
wb.save("output/soft_voter_base_predictions_visualization_with_accuracy.xlsx")
