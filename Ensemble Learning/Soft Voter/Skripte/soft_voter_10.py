...
model_info = {
    'ViTHybrid_10_SD': {
        'path': '/home/student_01/Projects/MA_Cahide/Bilderklassifizierung/Modelle/10/ViTHybrid_10_SD',
    ...
    'ViTHybrid_10_GAN': {
        'path': '/home/student_01/Projects/MA_Cahide/Bilderklassifizierung/Modelle/10/ViTHybrid_10_GAN',
        'processor': ViTHybridImageProcessor.from_pretrained("google/vit-hybrid-base-bit-384"),
        'custom_class': ViTHybridForImageClassification
    },
    
    'ViTHybrid_10_cutout': {
        'path': '/home/student_01/Projects/MA_Cahide/Bilderklassifizierung/Modelle/10/ViTHybrid_10_cutout',
        'processor': ViTHybridImageProcessor.from_pretrained("google/vit-hybrid-base-bit-384"),
        'custom_class': ViTHybridForImageClassification
    },
    
    'ViTHybrid_10_all': {
        'path': '/home/student_01/Projects/MA_Cahide/Bilderklassifizierung/Modelle/10/ViTHybrid_10_all',
        'processor': ViTHybridImageProcessor.from_pretrained("google/vit-hybrid-base-bit-384"),
        'custom_class': ViTHybridForImageClassification
    }
...
base_dir = ds
test_dataset = load_dataset('imagefolder', data_dir=base_dir)
print(test_dataset)
...
# Step 1: Convert logits to probabilities for each model
model_probs_dict = {name: F.softmax(preds, dim=1) for name, preds in model_predictions_dict.items()}

# Step 2: Average the probabilities across all models
# This creates a new tensor where each element is the average of the corresponding probabilities
# from all models for each class.
average_probs = torch.stack(list(model_probs_dict.values())).mean(dim=0)

# Step 3: Choose the class with the highest average probability for each data point
final_decisions = torch.argmax(average_probs, dim=1)

# Assuming true_labels contains the true class labels for the dataset
true_labels = torch.tensor(test_dataset['label']).to(device)

# Calculate the accuracy of soft voting predictions
accuracy = (final_decisions == true_labels).float().mean().item()

print(f"{datetime.now()}:Accuracy with Soft Voting: {accuracy * 100:.2f}%")
from sklearn.metrics import f1_score

# Convert predictions and true labels to CPU and to NumPy arrays for compatibility with sklearn
final_decisions_cpu = final_decisions.cpu().numpy()
true_labels_cpu = true_labels.cpu().numpy()

# Calculate F1 score
# 'average' parameter is crucial: it defines the type of averaging performed on the data:
...
f1_weighted = f1_score(true_labels_cpu, final_decisions_cpu, average='weighted')
f1_macro = f1_score(true_labels_cpu, final_decisions_cpu, average='macro')
f1_micro = f1_score(true_labels_cpu, final_decisions_cpu, average='micro')
...
ax.set_title(f'Confusion Matrix for Soft Voter', fontsize=20)
ax.tick_params(axis='x', labelrotation=45, labelsize=10)
ax.tick_params(axis='y', labelrotation=0, labelsize=10)
    
# Save confusion matrix
plt.tight_layout()
plt.savefig("output/soft_voter_10_confusion_matrix.png", dpi=300)
plt.close()
...
ax.set_xlabel('Predicted labels', fontsize=15)
ax.set_ylabel('True labels', fontsize=15)
ax.set_title(f'Confusion Matrix for Soft Voter', fontsize=18)
ax.tick_params(axis='x', labelrotation=90, labelsize=10)
ax.tick_params(axis='y', labelrotation=0, labelsize=10)
    
# Save normalized confusion matrix
plt.tight_layout()
plt.savefig("output/soft_voter_10_confusion_matrix_normalised.png", dpi=300)
plt.close()
print(f"{datetime.now()}: Confusion matrix for Hard Voter saved.")

print(f"{datetime.now()}:~~~saved confusion matrix~~~")
...
with open('output/soft_voter_10_logged_data.json', 'w') as f:
...
excel_path='output/soft_voter_10_predictions_visualization.xlsx'
...
wb = load_workbook("output/soft_voter_10_predictions_visualization.xlsx")
ws = wb.active
...
wb.save("output/soft_voter_10_predictions_visualization_formatted.xlsx")
from openpyxl import load_workbook
from openpyxl.styles import PatternFill
from openpyxl.utils import get_column_letter

# Load your workbook and select the first sheet
wb = load_workbook("output/soft_voter_10_predictions_visualization_formatted.xlsx")
ws = wb.active
...
# Save the workbook
wb.save("output/soft_voter_10_predictions_visualization_with_accuracy.xlsx")
