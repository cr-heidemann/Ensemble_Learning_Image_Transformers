...
plt.tight_layout()
plt.savefig("output/hard_voter_10_confusion_matrix.png", dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig("output/hard_voter_10_confusion_matrix_normalised.png", dpi=300)
plt.close()
print(f"{datetime.now()}: Confusion matrix for Hard Voter saved.")
...
with open('output/hard_voter_10_logged_data.json', 'w') as f:
...
excel_path='output/hard_voter_10_predictions_visualization.xlsx'
...
wb = load_workbook("output/hard_voter_10_predictions_visualization.xlsx")
ws = wb.active
...
prediction_cols = ['C', 'D', 'E', 'F', 'G']  # Add more columns as needed based on your file
...
wb.save("output/hard_voter_10_predictions_visualization_formatted.xlsx")
from openpyxl import load_workbook
from openpyxl.styles import PatternFill
from openpyxl.utils import get_column_letter

# Load your workbook and select the first sheet
wb = load_workbook("output/hard_voter_10_predictions_visualization_formatted.xlsx")
ws = wb.active

# Assuming column B contains the true labels
true_label_col = 'B'
# Assuming column C contains the hard voter predictions, and columns D onwards are individual model predictions
prediction_cols = ['C', 'D', 'E', 'F', 'G']  # Adjust based on your file
...
# Save the workbook
wb.save("output/hard_voter_10_predictions_visualization_with_accuracy.xlsx")
