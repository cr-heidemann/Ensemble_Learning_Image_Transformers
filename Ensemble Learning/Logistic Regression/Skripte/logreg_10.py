...
plt.tight_layout()
plt.savefig("output/log_reg_10_confusion_matrix.png", dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig("output/logreg_10_confusion_matrix_normalised.png", dpi=300)
plt.close()
print(f"{datetime.now()}: Confusion matrix for Hard Voter saved.")
...
with open('output/logreg_10_logged_data.json', 'w') as f:
...
excel_path = 'output/logreg_10_predictions_visualization.xlsx'
df.to_excel(excel_path, index=False)
print(f"{datetime.now()}: Results saved to {excel_path}")

# Apply formatting to Excel
wb = load_workbook("output/logreg_10_predictions_visualization.xlsx")
ws = wb.active
true_label_col = 'B'
prediction_cols = ['C', 'D', 'E', 'F', 'G']
...
wb.save("output/logreg_10_predictions_visualization_formatted.xlsx")

# Accuracy Calculation and Formatting
wb = load_workbook("output/logreg_10_predictions_visualization_formatted.xlsx")
ws = wb.active
...
wb.save("output/logreg_10_predictions_visualization_with_accuracy.xlsx")
